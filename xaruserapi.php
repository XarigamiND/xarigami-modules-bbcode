...
function bbcode_transform($text)
{
    ...
    //
    $magiclink = xarModGetVar('bbcode','magiclink');
    if ($magiclink == TRUE) {
        $text = preg_replace("#(^|[\n ]|[^\[img|url*]])([\w]+?://[^ \"\n\r\t<[]*)#is", "\\1[url]\\2[/url]", $text);
    ...
    $bbcode->addCode ('url', 'usecontent?', 'do_bbcode_url',
                        array ('usecontent_param' => 'default'),'link', array ('block', 'inline'), array ('link'));
